...
- contractStatus() view returning (uint256 rate, uint8 decimals, uint256 ethBalance, bool rescueToZeroEnabled, uint256 queuedAmount, uint256 queuedExecuteTime, address queuedRecipient, uint256 queuedRescueToZeroExecuteTime, uint256 maxSafeWei) — a convenience bundle exposing commonly-needed read values (rate, decimals, contract ETH balance, queued withdrawal and rescue timelock state, and the conservative max-safe msg.value bound used by buy()/previewBuy()). This is read-only and mirrors values already derivable by separate calls.
