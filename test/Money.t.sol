...
contract MoneyTest is Test {
    ...
    function testRescueERC20RejectsZeroRecipient() public {
        ...
        // capture balances before the attempted rescue
        uint256 moneyBalBefore = token.balanceOf(address(money));
        uint256 ownerBalBefore = token.balanceOf(owner);

        // owner attempts to rescue to the zero address and it must revert
        vm.prank(owner);
        vm.expectRevert();
        money.rescueERC20(token, address(0), 1);

        // balances must remain unchanged after the reverted attempt
        assertEq(token.balanceOf(address(money)), moneyBalBefore);
