...
contract MoneyTest is Test {
    ...
    function testBuyForMintsToRecipient() public {
        ...
        vm.prank(alice);
        vm.expectEmit(true, true, false, true);
        emit Money.Bought(alice, recipient, sendWei, expected, rate);
        vm.prank(alice);
    ...
    function testExecuteWithdrawalToRevertingRecipientPreservesQueuedState() public {
        ...
        // have bad contract queue a withdrawal (it is owner)
        ...
        // execute as attacker; recipient will revert so executeWithdrawal should revert and queued state preserved
        vm.expectRevert();
        money2.executeWithdrawal();

        // queuedAmount should still be set
