...
contract MoneyTest is Test {
    ...
    function testExecuteWithdrawalToRevertingRecipientPreservesQueuedState() public {
        ...
        // fund money2 so queued withdrawal can be executed
        vm.deal(address(this), 1 ether);
        payable(address(money2)).transfer(1 ether);

        // queue withdrawal as the BadRecipient (owner)
        bad.callQueueWithdrawal(money2, 1 ether);

        // fast forward time
        vm.warp(block.timestamp + 48 hours + 1);

        // execute withdrawal should revert when sending to bad recipient
        vm.expectRevert(bytes("Transfer failed"));
        money2.executeWithdrawal();

        // queued state should be preserved because transfer reverted after clearing state in executeWithdrawal
        // Note: Money.executeWithdrawal clears state before external call; therefore after revert, state will be reverted to pre-call values
        assertEq(money2.queuedAmount(), 1 ether);
    }

    // New test: verifies non-owner cannot call rescueERC20
    function testRescueERC20OnlyOwnerReverts() public {
        // deploy a simple ERC20 and mint tokens to the Money contract
        SimpleERC20 token = new SimpleERC20("TKN", "TKN");
        token.mint(address(money), 1000);

        // non-owner (alice) attempts to rescue tokens and should revert
        vm.prank(alice);
        vm.expectRevert();
        money.rescueERC20(IERC20(address(token)), alice, 1000);
    }
}
