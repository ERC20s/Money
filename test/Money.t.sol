...
contract MoneyTest is Test {
    ...
    // New test: ensure fallback() emits Deposit when called with non-empty calldata
    function testEmitDepositOnFallback() public {
        address sender = address(0xC0FFEE);
        uint256 amt = 1 ether / 1000;
    ...
    function testExecuteWithdrawalToRevertingRecipientPreservesQueuedState() public {
        ...
        // fund money2 so it can queue and attempt withdrawal
        vm.deal(address(this), 5 ether);
        payable(address(money2)).transfer(1 ether);

        // have the BadRecipient (owner of money2) queue a withdrawal of 1 ether
        bad.callQueueWithdrawal(money2, 1 ether);

        // advance time so withdrawal can be executed
        vm.warp(block.timestamp + 48 hours + 1);

        // attempting to execute should revert because BadRecipient rejects ETH
        vm.expectRevert(bytes("Transfer failed"));
        money2.executeWithdrawal();

        // queued state should be preserved after failed execute (reverted transaction)
