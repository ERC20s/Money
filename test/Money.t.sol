...
contract MoneyTest is Test {
    ...
    function testRescueERC20RejectsZeroToken() public {
        // ensure Money token balances unchanged when rescue is called with token == address(0)
        uint256 ownerBefore = money.balanceOf(owner);
        uint256 contractBefore = money.balanceOf(address(money));

        vm.prank(owner);
        vm.expectRevert(bytes("Token zero"));
        money.rescueERC20(IERC20(address(0)), owner, 1);

        // balances must be unchanged after the revert
        assertEq(money.balanceOf(owner), ownerBefore);
        assertEq(money.balanceOf(address(money)), contractBefore);
    }
